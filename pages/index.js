...
const ENV_ICO_MINT = process.env.NEXT_PUBLIC_ICO_MINT_TOKEN;
...
export default function Home() {
  ...
  const fetchIcoData = async () => {
    try {
      ...
      console.log("Failed to fetch ICO data:", error);
    }
  };
  ...
  const createIcoAta = async () => {
    try {
      ...
      console.log("Error creating ICO ATA:", error);
      alert("Failed to initialize ICO");
    }
  };
  ...
  const fetchUserTokenBalance = async () => {
    try {
      ...
      console.log("Failed to fetch user token balance:", error);
      setUserTokenBalance("0");
    }
  };
}
